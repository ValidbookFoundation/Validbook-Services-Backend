...
class FillStoryInChannelsAfterLogJob extends BaseObject implements Job
{
    ...
    public function execute($queue)
    {
        ...
        $yesterday = time() - (1 * 13 * 60 * 60);

        foreach ($channels as $id) {

            $channelStory = ChannelStory::find()
                ->where(['channel_id' => $id, 'story_id' => $this->storyId])
                ->andWhere(['between', 'story_created_at', $yesterday, time()])
                ->one();

            if (empty($channelStory)) {
                $model = new ChannelStory();
                $model->channel_id = $id;
                $model->story_id = $this->storyId;
                $model->story_created_at = $this->date;
